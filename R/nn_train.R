nn_train <- function(x, y, niter = 10L, learning.rate = 0.01, lambda,
  ...
  if(is.null(model)) {
    model = nn_fun(as.integer(d), as.integer(first.layer.width), as.integer(hidden.layer.width))
  }
  ...
  result <- train(model = model, train_iter = train_loader,
  ...
  yhat <- plogis(model$predict(train_features)$data$numpy())
