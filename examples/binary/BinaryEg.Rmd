...
```{r method_in_sample, message=FALSE, warning=FALSE, cache=FALSE}
...
```{r w2 distances}
names(interaction_adapt) <- 
  names(simple_adapt) <- 
  names(gamm_adapt) <- c("Selection",
                      # "Loc./Scale",
                         "MCP Net Projection",
                         "MCP Projection",
                         "Lasso Projection")
...
if ( !file.exists("w2plot_bin.rds") ) {
  ...
  w2plot$interaction <- plot.compare(interaction_adapt, post.bart$eta, 
                                     combinations, t(post.stan.full$theta), 
                       "w2", c("mean"), parallel = TRUE) 
  w2plot$gamm <- plot.compare(gamm_adapt, post.bart$eta, gammX, t(post.gamm$theta), 
                       "w2", c("mean"), parallel = TRUE) 
  w2plotData <- lapply(w2plot, function(i) i$data$mean)
  saveRDS( w2plotData, "w2plot_bin.rds" )
  w2plots <- lapply(w2plot, function(i) i$plot$mean)
} else {
  w2plotData <- readRDS( "w2plot_bin.rds" )
  w2plots <- lapply(w2plotData, function(ww) ggplot(ww, aes(x=nactive, y=dist, color = groups, group=groups )) +
...
if (!file.exists("mseplot_bin.rds")) {
  ...
  mseplot$interaction <- plot.compare(interaction_adapt, prob, combinations, t(post.stan.full$theta), 
                          "mse", c("mean"), parallel = TRUE, transform = plogis)
  mseplot$gamm <- plot.compare(gamm_adapt, prob, gammX, t(post.gamm$theta),
                          "mse", c("mean"), parallel = TRUE,transform = plogis)
  mseplotData <- lapply(mseplot, function(i) i$data$mean)
  saveRDS( mseplotData, "mseplot_bin.rds" )
  mseplots <- lapply(mseplot, function(i) i$plot$mean)
  
} else {
  mseplotData <- readRDS("mseplot_bin.rds" )
  mseplots <- lapply(mseplotData, function(mm) ggplot(mm, aes(x=nactive, y=dist, color = groups, group=groups )) +
        geom_line() + scale_color_jama() + labs(color ="Method") +
...
print(w2plots$simple)
print(mseplots$simple)
```
...
print(w2plots$interaction)
print(mseplots$interaction)
```
