...
\[Y_i \sim \text{Bernoulli}(p) \] with \[p = \text{logit}^{-1} \left (\sin\left(\frac{\pi}{8}x_{i,1}x_{i,2} + \pi/2 \right) + 2 I \left(x_{i,3}^2 > 6/4 \pi\right) +  2 I \left(x_{i,3}^2 \leq 6/4 \pi \right) \cos\left(2 x_{i,3}^2  + \pi/2\right) + \tanh(-x_{i,4}) + 0.25 x_{i,5}\right).\]
...
```{r posterior, message = FALSE, warning = FALSE, results="hide"}
...
print(xtable(mse.best.perform, digits=3, caption="MSE from true data generating probabilities"))
```
Clearly, the BART model fits best.
...
if( !file.exists("interact_adapt.rds") ) {
  interaction_adapt <- list(
                 ...
                 projection2 = W2L1(X=combinations, Y=post.bart$eta, 
                        theta=post.stan.full$theta, penalty="mcp", alpha=0.99,
                        nlambda = nlambda, lambda.min.ratio = lambda.min.ratio,
                        infimum.maxit=1, maxit = 1e3, gamma = gamma,
                        pseudo_observations = pseudo.observations, display.progress = TRUE,
                        penalty.factor = penalty.factor.full, method="projection",tol = 1e-3),
                 projection3 = W2L1(X=combinations, Y=post.bart$eta, 
  ...
  saveRDS(interaction_adapt, "interact_adapt.rds")
} else {
  interaction_adapt <- readRDS( "interact_adapt.rds")
}

if (!file.exists("simple_adapt.RDS")){
  simple_adapt <- list(
                     ...
                     projection2 = W2L1(X=X, Y=post.bart$eta, 
                        theta=post.stan$theta, penalty="mcp", alpha = 0.5,
                        nlambda = nlambda, lambda.min.ratio = lambda.min.ratio,
                        infimum.maxit=1, maxit = 1e3, gamma = gamma,
                        pseudo_observations = pseudo.observations, display.progress = TRUE,
                        penalty.factor = penalty.factor.simple, method="projection"),
                     projection3 = W2L1(X=X, Y=post.bart$eta, 
  ...
  saveRDS(simple_adapt, "simple_adapt.RDS")
} else {
  simple_adapt <- readRDS("simple_adapt.RDS")
}
...
names(interaction_adapt) <- names(simple_adapt) <- names(gamm_adapt) <- c("Selection",
                                                                        # "Loc./Scale",
                                                                        "MCP Net Projection",
                                                                        "MCP Projection",
                                                                        "Lasso Projection")
w2plot <- list(simple=NULL,
               interaction = NULL, 
               gamm = NULL)
if ( !file.exists("w2plot_bin.rds") ) {
  w2plot$simple <- plot.compare(simple_adapt, post.bart$eta, X, t(post.stan$theta), 
  ...
  w2plot$interaction <- plot.compare(interaction_adapt, post.bart$eta, 
                                     combinations, t(post.stan.full$theta), 
                       "w2", c("mean"), parallel = TRUE) 
  w2plot$gamm <- plot.compare(gamm_adapt[c(1,4)], post.bart$eta, gammX, t(post.gamm$theta), 
                       "w2", c("mean"), parallel = TRUE) 
  w2plotData <- lapply(w2plot, function(i) i$data)
  saveRDS( w2plotData, "w2plot_bin.rds" )
  w2plots <- lapply(w2plot, function(i) i$plot)
} else {
  w2plotData <- readRDS( "w2plot_bin.rds" )
  w2plots <- lapply(w2plotData, function(w) lapply(w, function(ww) ggplot(ww, aes(x=nactive, y=dist, color = groups, group=groups )) +
...
mseplot <- list(simple=NULL,
               interaction = NULL, 
               gamm = NULL)
if (!file.exists("mseplot_bin.rds")) {
  mseplot$simple <- plot.compare(simple_adapt, prob, X, t(post.stan$theta),
                        "mse", c("mean"), parallel = TRUE, transform = plogis)
  mseplot$interaction <- plot.compare(interaction_adapt, prob, combinations, t(post.stan.full$theta), 
                          "mse", c("mean"), parallel = TRUE, transform = plogis)
  mseplot$gamm <- plot.compare(gamm_adapt[c(1,4)], prob, gammX, t(post.gamm$theta),
                          "mse", c("mean"), parallel = TRUE,transform = plogis)
  mseplotData <- lapply(mseplot, function(i) i$data)
  saveRDS( mseplotData, "mseplot_bin.rds" )
  mseplots <- lapply(mseplot, function(i) i$plot)
  
} else {
  mseplotData <- readRDS("mseplot_bin.rds" )
  mseplots <- lapply(mseplotData, function(m) lapply(m, function(mm) ggplot(mm, aes(x=nactive, y=dist, color = groups, group=groups )) +
        geom_line() + scale_color_jama() + labs(color ="Method") +
...
print(w2plots$simple$mean)
print(mseplots$simple$mean)
```
...
print(w2plots$interaction$mean)
print(mseplots$interaction$mean)
```
...
print(w2plots$gamm$mean)
print(mseplots$gamm$mean)
```
...
gammXNew <- mgcv::predict.gam(post.gamm$model$jam, newdata=Xnew, type="lpmatrix")
...
```{r method_subset_sample, message=FALSE, warning=FALSE, cache=TRUE}
# penalty.factor.simple <- 1/colMeans(abs(post.stan$theta))
# penalty.factor.full <- 1/colMeans(abs(post.stan.full$theta))
# penalty.factor.gamm <- 1/colMeans(abs(post.gamm$theta))
penalty.factor.simple <- rep(1, ncol(post.stan$theta))
...
if ( !file.exists("w2plot_bin_subset.rds") ) {
  ...
  w2plot_subset$interaction <- plot.compare(interaction_adapt_subset, bartetaNew, 
                                     combinationsNew, t(post.stan.full$theta), 
                       "w2", c("mean"), parallel = TRUE) 
  w2plot_subset$gamm <- plot.compare(gamm_adapt_subset[c(1,4)], bartetaNew, gammXNew, t(post.gamm$theta), 
                       "w2", c("mean"), parallel = TRUE) 
  w2plotData_subset <- lapply(w2plot_subset, function(i) i$data)
  saveRDS( w2plotData_subset, "w2plot_bin_subset.rds" )
  w2plots_subset <- lapply(w2plot_subset, function(i) i$plot)
} else {
  w2plotData_subset <- readRDS( "w2plot_bin_subset.rds" )
  w2plots_subset <- lapply(w2plotData_subset, function(w) lapply(w, function(ww) ggplot(ww, aes(x=nactive, y=dist, color = groups, group=groups )) +
        geom_line() + scale_color_jama() + labs(color ="Method") +
        xlab("Number of active coefficients") + ylab("2-Wasserstein Distance") + theme_bw() +
        scale_x_continuous(expand = c(0, 0)) +
        scale_y_continuous(expand = c(0, 0))))
...
```{r mse distances new, cache=TRUE}
...
if (!file.exists("mseplot_bin_subset.rds")) {
  ...
  mseplot_subset$interaction <- plot.compare(interaction_adapt_subset, probNew, combinationsNew, t(post.stan.full$theta), 
                          "mse", c("mean"), parallel = TRUE, transform = plogis)
  mseplot_subset$gamm <- plot.compare(gamm_adapt_subset[c(1,4)], probNew, gammXNew, t(post.gamm$theta),
                          "mse", c("mean"), parallel = TRUE,transform = plogis)
  mseplotData_subset <- lapply(mseplot_subset, function(i) i$data)
  saveRDS( mseplotData_subset, "mseplot_bin_subset.rds" )
  mseplots_subset <- lapply(mseplot_subset, function(i) i$plot)
  
} else {
  mseplotData_subset <- readRDS("mseplot_bin_subset.rds" )
  mseplots_subset <- lapply(mseplotData_subset, function(m) lapply(m, function(mm) ggplot(mm, aes(x=nactive, y=dist, color = groups, group=groups )) +
        geom_line() + scale_color_jama() + labs(color ="Method") +
        xlab("Number of active coefficients") + ylab("MSE") + theme_bw() +
        scale_x_continuous(expand = c(0, 0)) +
        scale_y_continuous(expand = c(0, 0))))
...
```{r mse subset, echo=FALSE}
mse.best.perform_new <- data.frame(BART = mean((probNew - plogis(t(bartetaNew)))^2),
   'Simple Logistic' = mean((prob - plogis(Xnew %*% t(post.stan$theta)))^2),
   'Logistic with Interactions' = mean((prob - plogis(combinationsNew %*% t(post.stan.full$theta)))^2),
   GAMM = mean((qlogis(prob) - plogis(gammXNew %*% t(post.gamm$theta)))^2))
rownames(mse.best.perform_logit) <- "MSE"
print(xtable(mse.best.perform_new))
```
...
print(w2plots_subset$simple$mean)
print(mseplots_subset$simple$mean)
```
...
print(w2plots_subset$interaction$mean)
print(mseplots_subset$interaction$mean)
```
...
print(w2plots_subset$gamm$mean)
print(mseplots_subset$gamm$mean)
```
