...
if ( !file.exists(w2_file) ) {
  w2plot$bart$simple <- distCompare(simple_adapt_bart, list(mean = post.bart$eta),
                       method = "exact", quantity = c("mean"), parallel = TRUE) 
  w2plot$bart$interaction <- distCompare(interaction_adapt_bart, list(mean = post.bart$eta),
                       method = "exact", quantity = c("mean"), parallel = TRUE) 
  w2plot$gam$simple <- distCompare(simple_adapt_gam, list(mean = post.gamm$eta),
                       method = "exact", quantity = c("mean"), parallel = TRUE) 
  w2plot$gam$interaction <- distCompare(interaction_adapt_gam, list(mean = post.gamm$eta),
                       method = "exact", quantity = c("mean"), parallel = TRUE) 
  saveRDS( w2plot, "w2plot_bin.rds" )
} else {
  w2plot <- readRDS( w2_file )
  
}
w2plots <- lapply(w2plot, function(ll) lapply(ll, function(ww) plot(ww, ylab = "2-Wasserstein Distance")))
w2plots$bart <- set_equal_y_limits(w2plots$bart)
w2plots$gam <- set_equal_y_limits(w2plots$gam)
```
...
if (!file.exists(mse_file)) {
  mseplot$bart$simple <- distCompare(simple_adapt_bart, list(mean = prob),
                       method = "mse", quantity = c("mean"), parallel = TRUE,
                       transform = Y.list$invlink) 
  mseplot$bart$interaction <- distCompare(interaction_adapt_bart,  list(mean = prob),
                       method = "mse", quantity = c("mean"), parallel = TRUE,
                       transform = Y.list$invlink) 
  mseplot$gam$simple <- distCompare(simple_adapt_gam,  list(mean = prob),
                       method = "mse", quantity = c("mean"), parallel = TRUE,
                       transform = Y.list$invlink)  
  mseplot$gam$interaction <- distCompare(interaction_adapt_gam,  list(mean = prob),
                       method = "mse", quantity = c("mean"), parallel = TRUE,
                       transform = Y.list$invlink) 
  saveRDS( mseplot, mse_file )
} else {
  mseplot <- readRDS( mse_file )
}
mseplots <- lapply(mseplot, function(dd) lapply(dd, function(mm) plot(mm, ylab= "MSE")))
mseplots$bart <- set_equal_y_limits(mseplots$bart)
mseplots$gam <- set_equal_y_limits(mseplots$gam)
```
...
if ( !file.exists(w2subset_file) ) {
  w2plot_subset$bart$simple <- distCompare(simple_adapt_subset_bart, 
                                           target = list(mean = bartetaNew), 
                                           method = "exact", 
                                           quantity = "mean", parallel = TRUE) 
  w2plot_subset$bart$interaction <- distCompare(interaction_adapt_subset_bart, 
                                           target = list(mean = bartetaNew), 
                                           method = "exact", 
                                           quantity = "mean", parallel = TRUE) 
  w2plot_subset$gam$simple <- distCompare(simple_adapt_subset_gam,  
                                           target = list(mean = gametaNew), 
                                           method = "exact", 
                                           quantity = "mean", parallel = TRUE) 
  w2plot_subset$gam$interaction <- distCompare(interaction_adapt_subset_gam, 
                                           target = list(mean = gametaNew), 
                                           method = "exact", 
                                           quantity = "mean", parallel = TRUE) 
  saveRDS( w2plot_subset, w2subset_file )
} else {
  w2plot_subset <- readRDS( w2subset_file )
}
w2plots_subset <- lapply(w2plot_subset, 
                           function(ll) lapply(ll, function(ww) 
                           plot(ww, ylab = "2-Wasserstein Distance")))
w2plots_subset$bart <- set_equal_y_limits(w2plots_subset$bart)
w2plots_subset$gam <- set_equal_y_limits(w2plots_subset$gam)
...
if (!file.exists(mse_subset_file)) {
  mseplot_subset$bart$simple <- distCompare(simple_adapt_subset_bart, 
                                           target = list(mean = probNew), 
                                           method = "mse", 
                                           quantity = "mean", parallel = TRUE,
                                           transform = Y.list$invlink) 
  mseplot_subset$bart$interaction <- distCompare(interaction_adapt_subset_bart, 
                                           target = list(mean = probNew), 
                                           method = "mse", 
                                           quantity = "mean", parallel = TRUE,
                                           transform = Y.list$invlink) 
  mseplot_subset$gam$simple <- distCompare(simple_adapt_subset_gam,  
                                           target = list(mean = probNew), 
                                           method = "mse", 
                                           quantity = "mean", parallel = TRUE,
                                           transform = Y.list$invlink) 
  mseplot_subset$gam$interaction <- distCompare(interaction_adapt_subset_gam, 
                                           target = list(mean = probNew), 
                                           method = "mse", 
                                           quantity = "mean", parallel = TRUE,
                                           transform = Y.list$invlink) 
  saveRDS( mseplot_subset, mse_subset_file )
} else {
  mseplot_subset <- readRDS(mse_subset_file)
}
mseplots_subset <- lapply(mseplot_subset, 
                           function(ll) lapply(ll, function(ww) 
                           plot(ww, ylab = "MSE")))
mseplots_subset$bart <- set_equal_y_limits(mseplots_subset$bart)
mseplots_subset$gam <- set_equal_y_limits(mseplots_subset$gam)
...
w2plot <- list(bart = list (simple=NULL, interaction = NULL),
  ...
  w2plot$bart$simple <- distCompare(simple_adapt_bart, 
                                    list(mean = post.bart$eta),
                                    method = "exact", 
                                    quantity = c("mean"),
                                    parallel = TRUE) 
  w2plot$bart$interaction <- distCompare(interaction_adapt_bart, 
                                         list(mean = post.bart$eta),
                                         method = "exact", 
                                         quantity = c("mean"), 
                                         parallel = TRUE) 
  w2plot$gam$simple <- distCompare(simple_adapt_gam, 
                                   list(mean = post.gamm$eta), 
                                   method = "exact", 
                                   quantity = c("mean"), 
                                   parallel = TRUE) 
  w2plot$gam$interaction <- distCompare(interaction_adapt_gam, 
                                        list(mean = post.gamm$eta),
                                        method = "exact", 
                                        quantity = c("mean"), 
                                        parallel = TRUE) 

  w2plots <- lapply(w2plot, function(ll) 
      lapply(ll, function(ww) 
      plot(ww, ylab = "2-Wasserstein Distance")))
  w2plots$bart <- set_equal_y_limits(w2plots$bart)
  w2plots$gam <- set_equal_y_limits(w2plots$gam)
```
